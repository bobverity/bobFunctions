...
void SLIR_deterministic_odin_deriv(SLIR_deterministic_odin_pars *SLIR_deterministic_odin_p, double t, double *state, double *dstatedt, double *output) {
  ...
  {
    // delay block for S_delay
    const double delay_t = t - SLIR_deterministic_odin_p->dur_lag;
    if (delay_t <= SLIR_deterministic_odin_p->initial_t) {
      S_delay = 0;
    } else {
      double S;
      if (SLIR_deterministic_odin_p->odin_use_dde) {
        lagvalue_dde(delay_t, SLIR_deterministic_odin_p->delay_i_S_delay, SLIR_deterministic_odin_p->dim_delay_S_delay, SLIR_deterministic_odin_p->delay_state_S_delay);
      } else {
        lagvalue_ds(delay_t, SLIR_deterministic_odin_p->delay_i_S_delay, SLIR_deterministic_odin_p->dim_delay_S_delay, SLIR_deterministic_odin_p->delay_state_S_delay);
      }
  ...
  double I_delay;
  {
    ...
    if (delay_t <= SLIR_deterministic_odin_p->initial_t) {
      ...
      double I;
      if (SLIR_deterministic_odin_p->odin_use_dde) {
        lagvalue_dde(delay_t, SLIR_deterministic_odin_p->delay_i_I_delay, SLIR_deterministic_odin_p->dim_delay_I_delay, SLIR_deterministic_odin_p->delay_state_I_delay);
      } else {
        lagvalue_ds(delay_t, SLIR_deterministic_odin_p->delay_i_I_delay, SLIR_deterministic_odin_p->dim_delay_I_delay, SLIR_deterministic_odin_p->delay_state_I_delay);
      }
